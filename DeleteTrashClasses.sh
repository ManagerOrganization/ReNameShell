...
# class_search_dir="定义需要替换的类的查找目录"
class_search_dir=""
...
delete_trash_class_files_old() {
	...
	for class_file_path in `find ${class_search_dir} -name "${class_prefix}*${class_suffix}.[h|m]"`; do
		echo "删除文件>>>${class_file_path}"
		rm -f ${class_file_path}
	done

	for class_file_path in `find ${class_search_dir} -name "${class_prefix}*${class_suffix}.xib"`; do
		echo "删除文件>>>${class_file_path}"
		rm -f ${class_file_path}
	done
}

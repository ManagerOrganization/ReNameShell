...
# class_search_dir="(需要替换的类的查找目录)"
class_search_dir=""
# 配置文件
cfg_file="$(pwd)/configures/RenameClasses.cfg"

# project.pbxproj文件目录，需要替换该配置文件中的类的名称配置
#pbxproj_dir="需要替换该配置文件中的类的名称配置"
pbxproj_dir=""

# 定义该目录下的文件需要进行批量替换处理
# class_name_replace_dir="需要进行批量替换处理的文件目录"
class_name_replace_dir=""

# 定义该目录下的文件需要进行批量替换处理
#class_name_replace_support_dir="需要进行批量替换处理的文件目录"
#class_name_replace_support_dir=""
...
function add_prefix_suffix_to_class {
	...
	for (( i = 0; i < ${#config_content_array[@]}; i++ )); do
		...
		# 随机的前缀和后缀
        if [ -z $class_prefix ] || [ -z $class_suffix ]
        then

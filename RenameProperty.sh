...
property_name_replace_dir="/Users/mac/Desktop/ReName/ReName"
# 黑名单类目录
...
# 替换的属性配置文件
cfg_file="$(pwd)/configures/RenameProperties.cfg"
# 属性黑名单配置文件
custom_blacklist_cfg_file="$(pwd)/configures/CustomPropertiesBlackListConfig.cfg"
custom_blacklist_cfg_tmp_file="$(pwd)/configures/TmpCustomPropertiesBlackListConfig.cfg"
# 属性前缀,属性前缀需要特殊处理
prop_prefix="k"
# 属性后缀
prop_suffix="rn"
...
function rename_properties {
	...
	for (( i = 0; i < ${#rename_properties_config_content_array[@]}; i++ )); do
		...
		prop_prefix=`randstr`
		prop_suffix=`randstr`
		result_prop_name="${prop_prefix}${original_prop_name}${prop_suffix}"
		sed -i '{
